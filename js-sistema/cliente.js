...
// registro de talla
$('#frm_registro_talla').submit(function (event) {
    event.preventDefault();
    $('.error').remove();
    var formulario = $(this);
    var data = formulario.serialize();
    registro_abm(formulario, data);
    formulario[0].reset();
    $('#btn_cerrar_modal_talla').click();
    cargar_tallas();
});

function cargar_tallas() {
    $.post(site_url + "producto/get_sizes",
        function (data) {
            ...
            $.each(datos, function (i, item) {
                $('#talla_pantalon').append('<option value="' + item.descripcion + '">' + item.descripcion + '</option>');
                $('#talla_blusa').append('<option value="' + item.descripcion + '">' + item.descripcion + '</option>');
                $('#talla_saco').append('<option value="' + item.descripcion + '">' + item.descripcion + '</option>');
            });
        });
}
