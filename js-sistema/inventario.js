...
function lista_ingresos_adm() {
    $('#lista_ingreso_adm').DataTable({
        'lengthMenu': [[20, 60, 150, 250, 300], [20, 60, 150, 250, 300]],
        'paging': true,
        'info': true,
        'filter': false,
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    if (row.nombre_cliente != 'ANULADO') {
                        if (row.estado != '2') {
                            return '<a  role="button"  onclick="anular_inventario(this)" ><img width="30" height="30" src="' + base_url + '/assets/img/anular.png" title="Anular"></a>&nbsp;&nbsp;';
                        } else {
...
function anular_inventario(seleccionado) {
    var table = $('#lista_ingreso_adm').DataTable();
    var celda = $(seleccionado).parent();
    var rowData = table.row(celda).data();
    var id = rowData['id']

    swal({
            title: "ANULAR INVENTARIO " + rowData['id'],
            text: "Esta seguro de anular este dato?",
            type: "warning",
        ...
        function (isConfirm) {
            if (isConfirm) {
                $.ajax({
                     url: site_url + 'inventario/anular_inventario',
                    data: 'id=' + id,
                    type: 'post',
                    success: function (registro) {
                        if (registro) {
                            swal("Anulado!", "Inventario anulado", "success");
                            actualizarDataTable($('#lista_ingreso_adm'));
                        } else {
