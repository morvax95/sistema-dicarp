    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            string_tabla_fin_egreso = '</tr>' +
                '</table>';
            $("#label-flujo").text(response.monto_egreso_total - response.monto_ingreso_total + ' Bs.');
...
//exportar a pdf flujo caja
function imprimir_flujo_caja_detallado(element) {

    var inicio = $('#fecha-inicio-detalle').val();
    var fin = $('#fecha-fin-detalle').val();

   // location.href = site_url + 'flujo_caja/imprimir_flujo_caja_detallado' + '/' + inicio + '/' + fin;
    $.redirect(site_url + 'flujo_caja/imprimir_flujo_caja_detallado', {
        fecha_inicio: inicio,
        fecha_fin: fin,
