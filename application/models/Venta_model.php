...
class Venta_model extends CI_Model
{
    ...
    // Cambia el estado venta en proceso
    public function cambiar_estado_venta($id)
    {
    ...
    public function registro_venta()
    {
        ...
        $obj_venta['tipo_cambio'] = $this->input->post('tipo-cambio');
        ...
        // $detalle['descuento_producto'] = $this->input->post('descuento_producto_id[]');
        ...
        for ($index = 0; $index < $contador; $index++) {
            ...
            // $detalle_venta['descuento_producto'] = $detalle['descuento_producto'][$index];
    ...
    {
        $this->db->select("vp.descripcion,v.id,v.nota,vp.forma_pago,v.fecha, v.hora, v.subtotal, v.descuento, v.total, u.nombre_usuario, n.nro_nota, c.ci_nit, c.nombre_cliente, s.nit, s.nombre_empresa,s.sucursal, s.direccion, s.telefono, s.email,v.tipo_cambio,vp.monto ,vp.saldo");
        ...
        $resultado['datos_montos_venta_detalle'] = $this->obtener_montos_DetalleVenta($id);
        $resultado['datos_montos_venta_detalle_II'] = $this->obtener_montos_DetalleVenta_II($id);

        $resultado['datos_ultimo_montoTotal'] = $this->obtenerMontoTotal($id);
        $resultado['datos_ultimo_saldo'] = $this->obtenerSaldo($id);
        return $resultado;

    }

    function obtenerMontoTotal($id)
    {
        $this->db->select("id,sum(monto) as monto")
            ->from("venta_pago vp")
    ...
    {
        $this->db->select("vp.venta_id,id,saldo")
            ->from("venta_pago vp")
            ->where("vp.id=(select max(id) from venta_pago)");
        $datos_detalle = $this->db->get()->result();
        return $datos_detalle;
    }
    ...
    {
        $this->db->select("p.codigo_barra,max(vp.id),v.id,(d.cantidad + d.cantidad_produccion) as cantidad,d.precio_venta, p.nombre_item,d.estado_entrega,vp.monto as monto_plazos,vp.saldo")
            ->from("venta v, detalle_venta d, producto p,venta_pago vp")
            ->where("v.id = d.venta_id")
            ->where("d.producto_id = p.id")
            ->where("v.id = vp.venta_id")
            //->where("v.id = dt.venta_id")
            ->where("v.id", $idventa)
            ->group_by("v.id,d.precio_venta, p.nombre_item");
        $datos_detalle = $this->db->get()->result();
        return $datos_detalle;
    }

    function obtener_montos_DetalleVenta($idventa)
    {
        $this->db->select("v.descuento,v.total,sum(d.precio_venta) as precio_venta,vp.monto,vp.saldo")
            ->from("venta v, detalle_venta d,venta_pago vp")
            ->where("v.id = d.venta_id")
            ->where("v.id = vp.venta_id")
            ->where("v.id", $idventa);
    ...
    function obtener_montos_DetalleVenta_II($idventa)
    {
        $this->db->select("v.descuento,v.subtotal,v.total,sum(d.precio_venta) as precio_venta,sum(d.descuento_producto) as descuento_total_venta,sum(d.precio_venta) as suma_precio_venta")
            ->from("detalle_venta d,venta v")
            ->where("v.id = d.venta_id")
    ...
    {
        $this->db->select("v.id,p.nombre_item,dt.estado_entrega ,(dt.cantidad) as cantidad_total,c.nombre_cliente,vp.forma_pago");
