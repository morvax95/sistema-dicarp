    ...
    */
    ...
    public function registrar_privilegios_usuario($funciones, $id_usuario)
    {
        ...
        if ($this->db->trans_status() === FALSE) {
            $this->db->trans_rollback();
            return false;
            //$this->db->insert('acceso', array('usuario_id' => $id_usuario, 'menu_id' => 1));
        } else {
            $this->db->trans_commit();
            return true;
        }
...
//        $res = $this->db->query('select id from menu_cms where parent is null')->result();
    ...
    public function registrarRol()
    {
        ...
        $this->form_validation->set_rules('nombre_rol', 'nombre rol', 'trim|required|is_unique[cargo.descripcion]');
        $this->form_validation->set_error_delimiters('<p class="error">', '</p>');

        if ($this->form_validation->run() === true) {
            /** OBTENERMOS VALORES DE LOS INPUT **/
            $obj_data['descripcion'] = mb_strtoupper($this->input->post('nombre_rol'), "UTF-8");
            // $obj_data['fechaRegistro'] = date('Y-m-d');
            //$obj_data['estado'] = 1;
            $response['success'] = $this->db->insert('cargo', $obj_data);
        } else {
            foreach ($_POST as $key => $value) {
                $response['messages'][$key] = form_error($key);
            }
