...
class Flujo_caja extends CI_Controller
{
    ...
    public function detalle_excel()
    {
        ...
        $fila = 9;
        $total_filas_ingresos = 10;//$ingresos['row_total'];
        /*combinamos para crear columna INGRESO*/
        $cantidad_filas_ingresos = $this->obtener_cantidad_ingresos($ingresos);
    ...
    public function imprimir_flujo_caja_detallado()
    {
        $fecha_inicio = $this->input->post('fecha_inicio');
        $fecha_fin = $this->input->post('fecha_fin');
        $datos = $this->flujo->get_flujo_caja_detallado($fecha_inicio, $fecha_fin);
        $datos_egresos_efectivo = $this->flujo->get_egresos_efectivo($fecha_inicio, $fecha_fin);
        $datos_egresos_transferencia = $this->flujo->get_egresos_transferencia($fecha_inicio, $fecha_fin);

        $lista = $datos['datos_flujo_caja_detallado'];
        $lista_egresos_efectivo = $datos_egresos_efectivo['datos_egreso_efectivo'];
        $lista_egresos_transferencia = $datos_egresos_transferencia['datos_egreso_transferencia'];

        foreach ($lista as $row_detalle) {
            $usuario = $row_detalle->nombre_usuario;
        }
        foreach ($lista_egresos_efectivo as $row_detalle) {
            $monto_egresos = $row_detalle->monto;

        }
        foreach ($lista_egresos_transferencia as $row_detalle) {
            $monto_transferencia = $row_detalle->monto;

        }
        $total = $monto_egresos + $monto_transferencia;
        ...
        // Define el alias para el número de página que se imprimirá en el pie
        $this->pdf->AliasNbPages();
        $this->pdf->SetTitle("REPORTE EGRESOS");
        ...
        $this->pdf->Cell(50, 5, 'REPORTE DE EGRESOS', 0, 0, 'C');
        $this->pdf->SetFont('Arial', '', 9);
        $this->pdf->SetTextColor(0, 0, 0);/* volvemos a color de texto negro*/
        ...
        $this->pdf->Ln(5);
        /*  CANTIDAD  */
        $this->pdf->Cell(27, 5, utf8_decode(' Fecha Inicio  :  ' . $fecha_inicio . '                                  Fecha Fin : ' . $fecha_fin), 'LB');
        $this->pdf->Cell(165, 5, '', 'RB');
        $this->pdf->Ln(7);
        ...
        $this->pdf->Cell(142, 5, "DESCRIPCION", 1, 0, 'C');
        // $this->pdf->Cell(42, 5, "FORMA PAGO", 1, 0, 'C');
        $this->pdf->Cell(40, 5, "MONTO TOTAL", 1, 0, 'C');

        $this->pdf->Ln(5);
        $this->pdf->SetFont('Arial', '', 8);
        $cantidad_filas = 0;

        $lista_compras = $datos['datos_flujo_caja_detallado'];
        ...
        foreach ($lista_compras as $row_detalle) {
            ...
            $this->pdf->Cell(142, 4, utf8_decode($row_detalle->descripcion), $estilo, 0, 'C');
            $this->pdf->Cell(40, 4, utf8_decode($row_detalle->monto), $estilo, 0, 'C');
        ...
        $this->pdf->SetFont('Arial', '', 8);
        $this->pdf->Cell(30, 5, $monto_egresos, 1, 0, 'R');
        $this->pdf->Ln(6);
        ...
        $this->pdf->Cell(25, 5, 'EGRESOS Bs :', 1, 0, 'R');
        $this->pdf->SetFont('Arial', '', 8);
        $this->pdf->Cell(30, 5, $monto_transferencia, 1, 0, 'R');
