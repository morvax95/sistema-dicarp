...
class Proforma extends CI_Controller
{
    ...
    public function print_proforma()
    {
        ...
        $this->pdf->Image($var_img, 10, 10, 58, 26);
        /*  NIT Y NRO FACTURA   */
        ...
        $this->pdf->SetTextColor(0, 0, 0);/* volvemos a color de texto negro*/
        $this->pdf->Cell(60, 4, $datos_proforma["datos_proforma"]->direccion, 0, 0, 'C');
        ...
        $this->pdf->SetFont('Arial', 'B', 12);
        $this->pdf->SetTextColor(248, 000, 000);
        $this->pdf->Cell(55, 5, utf8_decode('Nº ' . $datos_proforma["datos_proforma"]->nro_proforma), 0, 0, 'C');
        $this->pdf->SetTextColor(0, 0, 0);/* volvemos a color de texto negro*/
        $this->pdf->SetFont('Arial', 'B', 7);
        $this->pdf->Cell(60, 4, 'Telf. ' . $datos_proforma["datos_proforma"]->telefono, 0, 0, 'C');
        $this->pdf->Cell(60, 4, '', 0);
        ...
        /*
                $convertidor= New ConvertidorDigito();
                $monto_literal = $convertidor->ValorEnLetras($datos_proforma["datos_proforma"]->total, " ");*/
        /**/
        $this->pdf->Ln(2);
        $this->pdf->SetFont('Arial', '', 8);
        ...
        /*  LUGAR Y FECHA ///// NIT CI*/
        $this->pdf->SetFont('Arial', 'B', 8);
        $this->pdf->Cell(27, 5, ' ', 'TL');
        $this->pdf->SetFont('Arial', '', 8);
        $fecha_literal = date('d', strtotime($datos_proforma["datos_proforma"]->nota)) . ' de ';
        $fecha_literal = $fecha_literal . $this->obtener_mes(date('m', strtotime($datos_proforma["datos_proforma"]->fecha))) . ' del ';
        $fecha_literal = $fecha_literal . date('Y', strtotime($datos_proforma["datos_proforma"]->fecha));
        ...
        $this->pdf->SetFont('Arial', '', 8);
        $this->pdf->Cell(165, 5, utf8_decode($datos_proforma["datos_proforma"]->nota), 'RB');
        $this->pdf->Ln(7);
        /**/
    ...
    /*  Imprimir solicitud trabajo  */
    public function print_solicitud_trabajo()
    {
        $venta_id = $this->uri->segment(3);
        $datos_proforma = $this->proforma_model->imprimir_proforma($venta_id);
         ...
         * el color de relleno predeterminado
         */
        $this->pdf->SetTitle("ORDEN TRABAJO");
        /* La variable $x se utiliza para mostrar un número consecutivo */
        ...
        $this->pdf->SetFont('Arial', 'B', 12);
        $this->pdf->SetTextColor(248, 000, 000);
        $this->pdf->Cell(60, 5, 'ORDEN  TRABAJO', 0, 0, 'C');
        $this->pdf->SetFont('Arial', '', 9);
        $this->pdf->SetTextColor(0, 0, 0);/* volvemos a color de texto negro*/
        $this->pdf->Cell(60, 4, $datos_proforma["datos_proforma"]->direccion, 0, 0, 'C');
        $this->pdf->Cell(60, 4, '', 0);
        ...
        $this->pdf->Cell(27, 5, utf8_decode('Señor(es)       :'), 'LB');
        $this->pdf->SetFont('Arial', '', 8);
        $this->pdf->Cell(165, 5, utf8_decode($datos_proforma["datos_proforma"]->nombre_cliente . '                                                                                                                       USUARIO : ' . $datos_proforma["datos_proforma"]->nombre_usuario), 'RB');
        $this->pdf->Ln(7);
        ...
        //un arreglo con alineacion de cada celda
        /* Encabezado de la columna*/
        $this->pdf->Cell(32, 5, "CODIGO", 1, 0, 'C');
        $this->pdf->Cell(130, 5, "PRODUCTO", 1, 0, 'C');
        $this->pdf->Cell(30, 5, "CANTIDAD", 1, 0, 'C');
        ...
        foreach ($detalle_proforma as $row_detalle) {
            ...
            $this->pdf->Cell(32, 4, utf8_decode($row_detalle->codigo_barra), $estilo, 0, 'C');
            $this->pdf->Cell(130, 4, utf8_decode($row_detalle->nombre_item), $estilo, 0, 'C');
            $this->pdf->Cell(30, 4, utf8_decode($row_detalle->cantidad), $estilo, 0, 'C');
            $this->pdf->Ln(4);
            $nro = $nro + 1;
        }
        while ($numero_items > $cantidad_filas) {
            ...
            $this->pdf->Cell(32, 4, '', $estilo, 0, 'C');
            $this->pdf->Cell(130, 4, '', $estilo, 0, 'C');
            $this->pdf->Cell(30, 4, '', $estilo, 0, 'L');
        ...
        $this->pdf->Ln(3);
        $this->pdf->Cell(27, 5, utf8_decode('                                           ENCARGADO TIENDA                                ' . '                                 ENCARGADO FABRICA   '), ' ');
        $this->pdf->Ln(5);
        $this->pdf->Cell(27, 5, utf8_decode('--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------'), ' ');
        $this->pdf->Ln(15);
